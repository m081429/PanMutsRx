...
function check_variable () {
	...
	then 
		if [ $exitcode -ne 0 ]
		then
			echo "script:$scriptname step:$step" |mail -s  "WorkFlow:Somatic Caller workflow Status:failed  RUNID:$runid Sample:$samplename exitcode: $exitcode step:$step " $email
			exit 100
		fi
	else
...
function annovar () {
	...
	then
			rm $gatkFilOut $vcfout.hg*_multianno.txt $vcfout.avinput
	fi
	mv $vcfout.hg*_multianno.vcf $vcfout.vcf
}
